...
def ard():
    db_version = "3440"
    return pyard.init(db_version, data_dir="/tmp/py-ard")


def test_no_mac(ard):
    assert ard.redux("A*01:01:01", "G") == "A*01:01:01G"
    assert ard.redux("A*01:01:01", "lg") == "A*01:01g"
    assert ard.redux("A*01:01:01", "lgx") == "A*01:01"
    assert ard.redux("HLA-A*01:01:01", "G") == "HLA-A*01:01:01G"
    assert ard.redux("HLA-A*01:01:01", "lg") == "HLA-A*01:01g"
    assert ard.redux("HLA-A*01:01:01", "lgx") == "HLA-A*01:01"


def test_remove_invalid(ard):
    assert ard.redux("A*01:01:01", "G") == "A*01:01:01G"


def test_mac(ard):
    assert ard.redux("A*01:AB", "G") == "A*01:01:01G/A*01:02"
    assert ard.redux("HLA-A*01:AB", "G") == "HLA-A*01:01:01G/HLA-A*01:02"
...
def test_redux(ard):
    ...
    for ex in expected["redux"]:
        ...
        assert ard.redux(glstring, ard_type) == expected_gl


def test_serology(ard):
    data_dir = os.path.dirname(__file__)
...
def test_mac_G(ard):
    ...
    with pytest.raises(InvalidAlleleError):
        ard._redux_allele("HLA-A*01:AB", "G")


def test_xx_code(ard):
    expanded_string = """
    ...
    """.strip()
    gl = ard.redux("B*40:XX", "G")
    assert gl == expanded_string


def test_xx_code_with_prefix(ard):
    expanded_string = """
    ...
    """.strip()
    gl = ard.redux("HLA-B*40:XX", "G")
    assert expanded_string == gl


def test_expand_mac(ard):
    mac_ab_expanded = ["A*01:01", "A*01:02"]
    assert ard.expand_mac("A*01:AB") == "/".join(mac_ab_expanded)

    mac_hla_ab_expanded = ["HLA-A*01:01", "HLA-A*01:02"]
    assert ard.expand_mac("HLA-A*01:AB") == "/".join(mac_hla_ab_expanded)

    mac_ac_expanded = ["A*01:01", "A*01:03"]
    assert ard.expand_mac("A*01:AC") == "/".join(mac_ac_expanded)

    mac_hla_ac_expanded = ["HLA-A*01:01", "HLA-A*01:03"]
    assert ard.expand_mac("HLA-A*01:AC") == "/".join(mac_hla_ac_expanded)


def test_redux_types():
    assert validate_reduction_type("G") is None
    assert validate_reduction_type("lg") is None
    assert validate_reduction_type("lgx") is None
    assert validate_reduction_type("W") is None
    assert validate_reduction_type("exon") is None
    with pytest.raises(ValueError):
        validate_reduction_type("XX")


def test_empty_allele(ard):
    with pytest.raises(InvalidAlleleError):
        ard.redux("A*", "lgx")


def test_fp_allele(ard):
    with pytest.raises(InvalidAlleleError):
        ard.redux("A*0.123", "lgx")


def test_empty_fields(ard):
    with pytest.raises(InvalidAlleleError):
        # : without any data
        ard.redux("DQA1*01:01:01:G", "lgx")


def test_invalid_serology(ard):
    # Test that A10 works and the first one is 'A*25:01'
    serology_a10 = ard.redux("A10", "lgx")
    assert serology_a10.split("/")[0] == "A*25:01"
    # And A100 isn't a valid typing
    with pytest.raises(InvalidAlleleError):
        ard.redux("A100", "lgx")


def test_allele_duplicated(ard):
    # Make sure the reduced alleles are unique
    # https://github.com/nmdp-bioinformatics/py-ard/issues/135
    allele_code = "C*02:ACMGS"
    allele_code_rx = ard.redux(allele_code, "lgx")
    assert allele_code_rx == "C*02:02"


def test_imgt_db_version(ard):
    assert ard.get_db_version() == 3440


def test_xx_codes_broad_split(ard):
    assert "DQB1*06" not in ard.redux("DQB1*05:XX", "lgx")


def test_cache_info(ard):
    # validate the default cache size
    assert ard._redux_allele.cache_info().maxsize == DEFAULT_CACHE_SIZE
    assert ard.redux.cache_info().maxsize == DEFAULT_CACHE_SIZE
    # validate you can change the cache size
    higher_cache_size = 5_000_000
    another_ard = pyard.init(
        "3440", data_dir="/tmp/py-ard", cache_size=higher_cache_size
    )
    assert another_ard._redux_allele.cache_info().maxsize == higher_cache_size
    assert another_ard.redux.cache_info().maxsize == higher_cache_size


def test_is_null(ard):
    # a null allele
    allele = "A*01:01N"
    assert ard.is_null(allele)
    # not null allele
    allele = "A*01:01"
    assert not ard.is_null(allele)
...
def test_default_redux_is_lgx(ard):
    allele = "A*24:BKKPV+A*26:03^B*15:BKNTS+B*15:07"
    lgx_redux = ard.redux(allele, "lgx")
    default_redux = ard.redux(allele)
    assert lgx_redux == default_redux


def test_mac_is_reversible(ard):
    mac_code = "A*68:AJEBX"
    expanded_mac = ard.expand_mac(mac_code)
    lookup_mac = ard.lookup_mac(expanded_mac)
    assert mac_code == lookup_mac
