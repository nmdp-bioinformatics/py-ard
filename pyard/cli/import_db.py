...
def get_imgt_version(version_number):
    if version_number:
        ...
        raise RuntimeError(
            f"{version_number} is not a valid IPD/IMGT-HLA database version number"
        )
    return "Latest"
...
def main():
    parser = argparse.ArgumentParser(
        ...
        Displays the list of available IPD/IMGT-HLA database versions.
        """,
    )
    parser.add_argument(
        "--list",
        dest="show_versions",
        action="store_true",
        help="Show Versions of available IPD/IMGT-HLA Databases",
    )
    parser.add_argument(
        "-i",
        "--ipd-version",
        dest="ipd_version",
        help="Import supplied IPD/IMGT-HLA DB Version",
    )
    parser.add_argument(
        "-d",
        "--data-dir",
    ...
    if args.show_versions:
        ...
        print("Available IPD/IMGT-HLA Versions:")
    ...
    try:
        ...
        sys.exit(1)
    print(f"Import complete for IPD/IMGT-HLA database version: {imgt_version}")
    ...
    if v2_to_v3_dict:
        ...
        print(
            f"Updated v2_mapping table with '{args.v2_v3_mapping}' mapping file for {imgt_version} IPD/IMGT-HLA database."
        )
