...
def expand_code(ard, args):
    ...
    elif ard.is_mac(code):
        try:
            allele_list = ard.expand_mac(code)
            print(allele_list)
        except InvalidMACError as e:
            print(e.message, file=sys.stderr)
    else:
        print("Not a XX code or MAC", file=sys.stderr)
...
def main():
    ...
    parser.add_argument(
        "--expand-xx", dest="expand_xx", help="Expand XX code to Allele List"
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.redux_type:
            ...
            for redux_type in VALID_REDUCTION_MODES:
